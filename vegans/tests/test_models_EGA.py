"""
Test for models with encoder, generator, adversariat structure.
"""
...
def test_init(gan, adv_dim, enc_dim):
    ...
    testgan = gan(generator=gen, adversariat=crit, encoder=enc, x_dim=x_dim, z_dim=z_dim, adv_type="Critic", folder=None)

    # TEST DISCRIMINATOR
    disc = generate_net(in_dim=adv_dim, last_layer=torch.nn.Sigmoid, out_dim=1)
    testgan = gan(generator=gen, adversariat=disc, encoder=enc, x_dim=x_dim, z_dim=z_dim, folder=None)
...
def test_fit_vector_feature_loss(gan, adv_dim, enc_dim):
    ...
    crit = generate_net(in_dim=adv_dim, last_layer=torch.nn.Identity, out_dim=1)
    testgan = gan(
        generator=gen, adversariat=crit, encoder=enc, x_dim=x_dim, z_dim=z_dim,
    ...
    disc = generate_net(in_dim=adv_dim, last_layer=torch.nn.Sigmoid, out_dim=1)
    testgan = gan(
        generator=gen, adversariat=disc, encoder=enc, x_dim=x_dim, z_dim=z_dim,
